...
def datasave(p:parameters):
    ...
    E_perp = p.E_perp[None, None, :]

    r,theta,E_perp = np.broadcast_arrays(r,theta,E_perp)
    ...
    data = np.column_stack([
        ...
        E_perp.ravel(),p.E.ravel(),p.trapped.ravel()
    ])
    ...
    columns = ['r', 'theta', 'v_perp before resonance', 'v_para', 'B', 'w_cyc', 'k_para', 'ddnu before resonance'
               ...
               ,'Second order tau imag','Third order tau real','Third order tau imag','Third order SR tau real','Third order SR tau imag','Pitch'
               ,'Pitch Plus','Pitch Minus','Perpendicular Energy','Total Energy','Trapped Indicator']
